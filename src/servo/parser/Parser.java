...
public class Parser {
    ...
    // TODO: Allow VAR and CONST Pointers
    private Stmt declaration() {
        try {
            if (match(VAR, LET, CONST)) return varDeclaration();
            return statement();
    ...
    // Does not allow for const declarations to be made in a for loop.
    ...
    private Expr factor() {
        ...
        while (match(SLASH, STAR, MODULO, POW)) {
            Token operator = previous();
            Expr right = unary();
            expr = new Expr.Binary(expr, operator, right);
        }
