...
func TestStringExpressions(t *testing.T) {
	tests := []compilerTestCase{
		{
			input:             `"Servo"`,
			expectedConstants: []interface{}{"Servo"},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpPop),
			},
		...
		{
			input:             `"Servo" + "VirtualMachine"`,
			expectedConstants: []interface{}{"Servo", "VirtualMachine"},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
				code.Make(code.OpAdd),
...
func TestCompilerScopes(t *testing.T) {
	...
	if len(compiler.scopes[compiler.scopeIndex].instructions) != 1 {
		t.Errorf("instructions lenfth wrong. got=%d", len(compiler.scopes[compiler.scopeIndex].instructions))
	}
	...
	if previous.OpCode != code.OpMul {
		t.Errorf("previousInstruction.OpCode wrong. got=%d, want=%d", previous, code.OpMul)
	}
}
